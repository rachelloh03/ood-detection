...
class GaussianMixtureWithScore:
    ...
    find the score of the data.

    If score_type is "md", finds the minimum Mahalanobis distance to the cluster centers.
    If score_type is "density", computes -log p(x) under a Gaussian Mixture Model.
    """
    ...
    def __init__(
        ...
        diagonal_cov: bool = True,
        random_state: int = 42,
        score_type: Literal["md", "density"] = "density",
    ):
        self.n_components = n_components
        self.dim = dim
        self.random_state = random_state
        ...
        self.__name__ = (
            f"GaussianMixtureWithScore(n_components={n_components}, dim={dim},"
            f"diagonal_cov={diagonal_cov}, score_type={score_type})"
        )
    ...
    def fit(self, x, tolerance: float = 1e-4):
        ...
        """
        if self.score_type not in ["md", "density"]:
        ...
        self.gm.fit(
            ...
            tolerance=tolerance,
            save_filepath=(
                f"gaussian_mixture_with_score_{self.n_components}_{self.dim}"
                f"{self.diagonal_cov}_{self.score_type}"
            ),
        )  # x: (N, D)
    ...
    def transform(self, x):
        ...
        """
        return (
            self.density(x) if self.score_type == "density" else self.min_md(x)
        )  # (N,)
