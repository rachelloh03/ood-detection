...
Experiment 1: Find the best pooling function and subsampling transformation.
Experiment 2: Find the best layer and GMM configuration.
"""
...
from extract_layers.pooling_functions import (
    pool_last_k_tokens,
    pool_mean_std,
    norm_weighted_mean_std,
)
...
def load_actual_datasets():
    ...
    print("len(id_train_base_dataset)", len(id_train_base_dataset))
    print("len(id_test_base_dataset)", len(id_test_base_dataset))
    print("len(ood_base_dataset)", len(ood_base_dataset))
...
def generate_all_transformations_experiment_2(
    ...
    pca_components_to_try=[50, 80, 100],
    diagonal_cov_to_try=[True, False],
):
    """Experiment 2: Find the best layer and GMM configuration."""
    all_transformations = []
    for layer in range(13, 21):
                    ...
                    all_transformations.append(
                        (
                            Transformations(
                                [
                                    extract_layer_transformation(
                                        model, norm_weighted_mean_std, [layer]
                                    ),
                                    StandardScaler(),
...
if __name__ == "__main__":
    ...
    stats_file = "results/stats_experiment_2.json"
    all_stats = []
